...
# Script to install cloudmesh client using pip in a virtualenv on Ubuntu Trusty
# as installed by Comet VC Tutorial. If required development tools are not
# installed they will be added.

# NOTE: This script does not currently add 'supported' version of Python 2.7 to
#       Ubuntu Trusty.
...
apt show build-essential &> /dev/null
if [[ $? -ne 0 ]]; then
